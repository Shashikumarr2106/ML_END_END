...
import os,sys 
import pandas as pd
from sensor import utils  ##yaml file only
import numpy as np
...
class DataValidation:
    ...
    def __init__(self,
        ...
        try:
            ...
            self.validation_error=dict()##file
    ...
    def drop_missing_values_columns(self,df:pd.DataFrame,report_key_name:str)->Optional[pd.DataFrame]:
        ...
        try:
            ...
            threshold = self.data_validation_config.missing_threshold ##0.3
            null_report = df.isna().sum()/df.shape[0] ##null percentage ex 0.56=56 perc
            #selecting column name which contains null
            logging.info(f"selecting column name which contains null above to {threshold}")
            drop_column_names = null_report[null_report>threshold].index ##column name drop ,missing percetage greater than 30 per
    ...
    def data_drift(self,base_df:pd.DataFrame,current_df:pd.DataFrame,report_key_name:str):
        try:
            ...
            raise SensorException(e, sys)
    ##calling one by one all function
